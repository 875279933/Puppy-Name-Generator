...
countSpan.textContent=`${favs.length} имя${favs.length===1?'':favs.length>=2&&favs.length<=4?'':''} выбрано${favs.length===1?'':favs.length>=2&&favs.length<=4?'ы':''}`;
}

function compareNames(){
const favs=getFavs();
if(!favs.length){alert('Нет выбранных имен. Нажмите на иконку сердца у любого имени, чтобы добавить его в свой список!');return}
localStorage.setItem('pupCompare',JSON.stringify(favs));
window.location.href='comparison-page.html';
}

function copyLink(el){
navigator.clipboard.writeText('https://puppynamegenerator.net/ru/').then(()=>{
el.classList.add('copied');
el.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg> Скопировано!';
setTimeout(()=>{el.classList.remove('copied');el.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2h11c.1 0 2-.9 2-2V7c0-1.1-.9 2-2-2zm0 16H8V7h11v14z"/></svg> Скопировать ссылку'},2000)
})
}
