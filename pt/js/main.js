...
countSpan.textContent=`${favs.length} nome${favs.length!==1?'s':''} selecionado${favs.length!==1?'s':''}`;
}

function compareNames(){
const favs=getFavs();
if(!favs.length){alert('Nenhum nome selecionado. Clique no ícone do coração em qualquer nome para adicionar à sua lista!');return}
localStorage.setItem('pupCompare',JSON.stringify(favs));
window.location.href='comparison-page.html';
}

function copyLink(el){
navigator.clipboard.writeText('https://puppynamegenerator.net/pt/').then(()=>{
...
setTimeout(()=>{el.classList.remove('copied');el.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2h11c.1 0 2-.9 2-2V7c0-1.1-.9 2-2-2zm0 16H8V7h11v14z"/></svg> Copiar link'},2000)
})
}
